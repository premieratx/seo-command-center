...
export default function ChatbotTestPanel() {
  ...
  return (
    <div className="flex flex-col h-[70vh] max-h-[720px] bg-zinc-50 border border-zinc-200 rounded-lg overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-200">
        <div>
          <div className="text-sm font-semibold text-zinc-900">🛥️ Test chat (guided flow preview)</div>
          <div className="text-[10px] text-zinc-500">
            Uses the knowledge base + inline widgets. Submitting the contact form creates a REAL lead in the Lead Dashboard.
          </div>
        </div>
        <button onClick={reset} className="text-[10px] text-zinc-400 hover:text-zinc-900 border border-zinc-200 rounded px-2 py-1">
          Reset
        </button>
      </div>
                ...
                <div
                  className={`max-w-[85%] rounded-2xl px-3.5 py-2 text-[13px] whitespace-pre-wrap ${
                    m.role === "user"
                      ...
                      : "bg-white border border-zinc-200 text-zinc-200 rounded-bl-sm"
                  }`}
                >
                  {m.content}
                </div>
      ...
      <form
        ...
        className="flex gap-1.5 border-t border-zinc-200 p-2 bg-zinc-50"
      >
        <input
          value={input}
          ...
          disabled={sending}
          className="flex-1 bg-white border border-zinc-200 text-sm text-zinc-900 rounded px-3 py-2 placeholder-zinc-600 focus:outline-none focus:border-blue-500"
        />
        <button
          type="submit"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
          {opts.map((o) => (
            <button
              ...
              className="text-[12px] bg-zinc-200 hover:bg-blue-600 hover:text-white border border-[#333] rounded-lg px-2.5 py-2 text-zinc-200 transition-colors"
            >
              {o}
            </button>
          ))}
    ...
    <div className="bg-white border border-zinc-200 rounded-xl p-3">
      ...
      <div className="flex gap-1.5">
        <input
          ...
          className="flex-1 bg-zinc-50 border border-zinc-200 text-sm text-zinc-900 rounded px-2.5 py-2 focus:outline-none focus:border-blue-500"
        />
        <button
          disabled={!v}
          onClick={() => {
...
function ContactFormCard({
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-zinc-200 rounded-xl p-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <input
          ...
          className="bg-zinc-50 border border-zinc-200 text-xs text-zinc-900 rounded px-2.5 py-1.5 placeholder-zinc-600 focus:outline-none focus:border-blue-500"
        />
        <input
      ...
      <input
        ...
        className="w-full bg-zinc-50 border border-zinc-200 text-xs text-zinc-900 rounded px-2.5 py-1.5 placeholder-zinc-600 focus:outline-none focus:border-blue-500"
      />
      <input
        type="tel"
        placeholder="Phone"
